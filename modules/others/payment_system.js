                      ...
                      if(alldata){
                        ...
                        try{
                          client.channels.fetch("943566379477520414").catch(e => {console.warn(e.stack ? String(e.stack).grey : String(e).grey)}).then(ch=>{
                            ch.send({content: `<a:tick:927612765382782987> **Deleted the Bot:** ${bot.user} | ${bot.user.tag} (\`${bot.user.id}\`)\n> **BOT-ID:** \`${botid}\`, **Server:** \`${serverid}\``})
                            try{
                              member.send({content: `<a:tick:927612765382782987> **Stopped your Bot:** ${bot.user} | ${bot.user.tag} (\`${bot.user.id}\`)\n> **This is because you need to pay again / stopped paying us / left the Server!**`})
                            }catch (e){
                              console.log(e)
                            }
                          })

...
const Roles = {
    FounderId: "952581433162096661",
    OwnerRoleId: "952581434185510953",
    CoOwnerRoleId: "952581435150204960",
    ChiefHumanResources: "952581435947110532",
    HumanResources: "952581437259923506",
    AdminRoleId: "952581440334356530",
    ModRoleId: "952581442200801290",
    ChiefSupporterRoleId: "952581444054712341",
    ChiefBotCreatorRoleId: "952581445120053309",
    BotCreatorRoleId: "952581445950525440",
    SupporterRoleId: "952581446340575243", 
    NewSupporterRoleId: "952581447837945866"
}
const overflows = [
    "952581592965066833",
    "952581594290462841"
...
module.exports = client => {
    let TicketChannelID = "952581647788806224";
    let RecoverChannelId = "952581625034735716";
            ...
            for(const deleteChannel of toDeleteChannels.map(d => d.channel)) {
                ...
                if(channel) {
                    ...
                    try {
                        ...
                        if (!userid && channel.parent && channel.parent.id == "952581571599269928")
                    ...
                    try {
                        ...
                        create_transcript_buffer([...messageCollection.values()], channel, channel.guild).then(async path => {
                            try { // try to send the file
                                ...
                                await client.channels.fetch("952581617682100224").then(async ch => {
                                    try {
        ...
        if(interaction.customId == "closeticket"){
            ...
            setTimeout(async() => {
                if(channel) {
                    ...
                    let parent1 = channel.guild.channels.cache.get("952581604658782350");//for closed ticket(s)
                    let parent2 = channel.guild.channels.cache.get("952581603245293668");
                    ...
                    if(parent1 && parent1.children.size < 50) {
                        ...
                        await channel.setParent("952581603245293668", {lockPermissions:false}).catch(()=>{});
                    }
                    await channel.permissionOverwrites.set([
                        {id: channel.guild.id, deny: [Discord.Permissions.FLAGS.VIEW_CHANNEL,Discord.Permissions.FLAGS.SEND_MESSAGES,Discord.Permissions.FLAGS.VIEW_CHANNEL]}
                    ]).catch(()=>{});
            ...
            if(menuIndex.type){
                ...
                switch(menuIndex.type){
                    case "SYSTEMBOTS" : {
                        ...
                        let parentId = "952581571599269928";
                        ...
                        guild.channels.create(channelname.substring(0, 32), {
                            ...
                            ch.send({content: `<@${user.id}> <a:right_arrow_yellow:947541936762388590> ${ticket.adminroles.map(r => `<@&${r}>`).join(" | ")} <:arrowl:938884347166064700>`, 
                            embeds: [
                    ...
                    case "MusicBots" : {
                        ...
                        let parentId = "952581573985849424";
                        const parentChannel = guild.channels.cache.get(parentId) || await guild.channels.fetch(parentId).catch(()=> {});
                        if(!parentChannel) return interaction.reply({ content: ":x: Could not find ticket parent", ephemeral: true })
                    ...
                    case "WAITINGROOMBOT" : {
                        ...
                        let parentId = "952581579677532221";
                        const parentChannel = guild.channels.cache.get(parentId) || await guild.channels.fetch(parentId).catch(()=> {});
                        if(!parentChannel) return interaction.reply({ content: ":x: Could not find ticket parent", ephemeral: true })
                    ...
                    case "24_7_MUSIC_BOT" : {
                        ...
                        let parentId = "952581572748529684";
                        const parentChannel = guild.channels.cache.get(parentId) || await guild.channels.fetch(parentId).catch(()=> {});
                        if(!parentChannel) return interaction.reply({ content: ":x: Could not find ticket parent", ephemeral: true })
                    ...
                    case "MODMAILBOT": {
                        ...
                        let parentId = "952581582995197952";
                        const parentChannel = guild.channels.cache.get(parentId) || await guild.channels.fetch(parentId).catch(()=> {});
                        if(!parentChannel) return interaction.reply({ content: ":x: Could not find ticket parent", ephemeral: true })
                    ...
                    case "SOURCECODE" : {
                        ...
                        guild.channels.create(channelname.substring(0, 32), {
                            ...
                            ch.send({content: `<@${user.id}> <a:right_arrow_yellow:947541936762388590> ${ticket.adminroles.map(r => `<@&${r}>`).join(" | ")} <:arrowl:938884347166064700>`, embeds: [
